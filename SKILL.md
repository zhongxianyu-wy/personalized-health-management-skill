...
## 数据库边界（PUA 核心）
- **数值**（OR/PPV/LR/先验/后验/VoI/sens/spec/价格）→ `cancerrisk/json/` + `pricing/json/`，**脚本算，LLM 不碰**。
...
5. 🔴 **CP4 结构化 + 5 artifact**：`... --stop-after health-summary-api` → `finalize_structured_summary.py` 结构化 → `... --person-id <id> --stop-after report-artifacts`（snapshot/VoI/归档自动）→ **LLM 产 5 section artifact**（见下表，**LLM+知识库提取异常+推荐筛查**；数值字段留空下游兜底）→ `... scripts/assemble_package.py --package <out>/artifacts/package_tiers.json --skill-root <skill_root>`
6. **最终报告**：`... --person-id <id>`（不带 stop-after）→ exit 0 → `report.html` 就绪。

## 5 section artifact（LLM 产，落 `<out>/artifacts/`）
| artifact | schema | LLM 产（读知识库） | 留空（下游脚本算） |
|---|---|---|---|
| `timeline_tiers.json` | `{priority/important/maintain:[{item_name,interval,rationale}]}` | 读 `cancer_followup_rules.json`(癌种复查) + `异常指标复查推荐.md`(任意异常→复查) + snapshot/health；自定三档+方法+rationale | — |
| `x_addons.json` | `[{risk_source,risk_level_tag,risk_level_label,method,interval,price_range,clinical_value}]` | risk_source/method/interval/tag/clinical_value（任意异常含乳腺/妇科/心电） | posterior/cancer_name（`_enrich_x_addons` 权威回填） |
| `package_tiers.json` | `[{name,price_range,includes[],note,recommended}]` 恒3档，仅1档 recommended=true | name/includes(**用 `08_pricing.json` 项目名**)/note(患者面向)/recommended | price_range（`assemble_package` Σmid，含¥） |
...
## 产物
用户交付物：`report.html`。审计 artifacts（`report.json`/`snapshot_risk.json`/`voi_ranking.json`/`cp3_audit_result.json`/`manifest.json` 等）落 `<out>/artifacts/`。归档落 `<cwd>/output/<person_id>/`（经 `archive_manager.py`，自动）。
