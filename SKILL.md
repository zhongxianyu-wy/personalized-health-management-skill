...
> | `x_addons.json` | `[{risk_source,risk_level_tag(danger/warning/info),risk_level_label,method,interval,price_range,clinical_value}]` | ≥1 行 | 异常复查MD + pricing JSON | **LLM 提取+推荐**：risk_source/method/interval/tag/clinical_value 全 LLM（任意异常含乳腺/妇科/心电）；**posterior_probability / cancer_name 留空**（LLM 不填数值），由 `_enrich_x_addons` 下游按 cancer 权威回填——匹配 snapshot 癌种则覆盖，未匹配/非癌项则清除（防 LLM 编造后验） |
> | `package_tiers.json` | `[{name,price_range,includes[],note,recommended}]` 恒 3 档，**recommended 每档必填 bool**，仅 1 档 true | 3 档 | 套餐三档MD + pricing JSON | **LLM 推荐**：name/includes/note/recommended 全 LLM（读套餐三档MD + pricing）；**includes[] 必须用 pricing DB (`08_pricing.json`) 的项目名/别名**（如「无痛肠镜/LDCT/甲状腺超声」），否则 `assemble_package` 匹配不上→价格失真；**price_range 由 `assemble_package.py` Σmid 下游算（含 ¥ 前缀，不手填）**；**note 是患者面向定位文案，不写内部脚本/实现细节** |
...
> | `long_term_intervention.json` | `{genetic_management[](仅brca positive),lifestyle[](list-of-str)}` **仅这两个字段** | lifestyle≥1 | 07预防MD | **LLM 推荐**：genetic_management（仅 BRCA 阳性，读遗传高危证据）+ lifestyle（读 07预防MD）全 LLM；**lifestyle 为 list-of-str**（如「戒烟限酒」），勿加 specialist_followup/monitoring_plan 等模板不渲染的额外字段（专科随访并入 lifestyle/genetic） |
>
